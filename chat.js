...
(function () {
  ...
  function faqAnswer(text) {
    ...
    if (has('발생원이뭐', '발생원이란', '발생원뭐', '발생원종류', '발생원설명', '어디서번식', '어디서생겨', '모기어디서')) {
      return R('발생원은 모기가 알을 낳고 번식하는 시설·환경이에요. 김해는 정화조·개인하수/오수처리·축산·저수지·폐타이어·수경시설·공중화장실·도시공원 등 16종 약 15,204곳을 반영합니다. 공통점은 "고인 물"이에요.', ['발생원이 몇 개야?', '가장 위험한 동네는?', '모기 예방법 알려줘']);
    }
    if (has('발생원몇', '발생원이몇', '몇개소', '몇곳', '시설몇', '총몇', '몇종')) {
      return R('김해시 전체 발생원은 16종 약 15,204곳이에요. 정화조·개인하수/오수처리시설이 가장 많고(청소 이력은 고유 시설로 보정), 축산농가 763곳, 공중화장실 563곳, 도시공원 245곳 등이 포함돼요.', ['발생원이 뭐야?', '가장 위험한 동네는?', '데이터는 어디서 구했어?']);
    }
  ...
  const FAQ_POOL = [
    // 구역
    '가장 위험한 동네는?', '가장 안전한 동네는?', '오늘 우리 동네 위험해?', '동네 목록 보여줘', '장유 오늘 몇 점?', '회현동은 어때?', '오늘 안전한 동네 추천해줘',
    // 개념·모델
    '모기지수가 뭐야?', '등급 기준이 어떻게 돼?', '몇 점부터 위험해?', '어떻게 계산해?', '왜 김해만 정밀 모델이야?', '검증은 어떻게 했어?', '정확도는 얼마야?', '얼마나 자주 갱신돼?', '너 AI야?', 'Brière가 뭐야?',
    // 발생원·데이터
    '발생원이 뭐야?', '발생원이 몇 개야?', '모기는 어디서 번식해?', '정화조가 왜 위험해?', '폐타이어는 왜?', '저수지는 왜?', '축사는 왜 위험해?', '데이터는 어디서 구했어?',
    // 예방·행동
    '모기 예방법 알려줘', '어떤 기피제를 써?', '기피제는 얼마나 자주 발라?', '물렸을 때는?', '모기 물린 자국은 왜 남아?', '모기 활동 시간은?', '해질녘에 왜 많아?', '산책은 어디가 좋아?', '어느 공원이 안전해?', '모기장 효과 있어?', '모기향 효과 있어?', '살충제 뿌려도 돼?', '모기 쫓는 식물 있어?',
    // 생태
    '모기 수명이 어떻게 돼?', '암컷만 물어?', '수컷도 물어?', '왜 나만 물려?', '왜 가려운 거야?', '모기에 물리면 왜 부어?', '모기 박멸 가능해?', '모기 소리는 왜 나?',
    // 날씨·계절
    '왜 여름에 모기가 많아?', '겨울에도 모기 있어?', '비 오면 모기 많아?', '더우면 모기 많아?', '습하면 모기 많아?', '왜 어제보다 점수가 달라?', '기후변화랑 관련 있어?',
    // 감염병
    '매개모기 종류는?', '일본뇌염이 위험해?', '뎅기열이 뭐야?', '말라리아도 옮겨?',
    // 대상별·생활
    '아기도 모기 물려?', '강아지도 모기 물려?', '무슨 색 옷이 좋아?', '술 마시면 더 물려?', '집 안 모기 어떻게 없애?', '실내에서도 물려?', '캠핑 갈 때 조심할 점?',
    // 지도·행정
    '지도 어떻게 봐?', '이 사이트 뭐야?', '방제는 어떻게 해?', '방제 우선순위는 어떻게 정해?', '유문등이 뭐야?', '연막소독 효과 있어?', '장구벌레가 뭐야?', '모기 많으면 어디 신고해?',
  ];
  // seed(관련 질문) 1개 + POOL에서 무작위로 채워, 매번 다른 4개를 만든다.
  function diversify(seed, currentQ) {
    ...
    shuffle(FAQ_POOL).forEach((q) => { if (out.length < 4) add(q); });     // 나머지 랜덤
    return shuffle(out);
  }
