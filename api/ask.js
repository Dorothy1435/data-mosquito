...
// 무료 티어 모델. 필요하면 이름만 바꾸면 된다(예: gemini-2.5-flash).
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      res.status(200).json({ answer: '⚠️ 잠시 답변을 가져오지 못했어요. 잠시 후 다시 시도해 주세요.', ok: false });
      return;
    }
    const data = await r.json();
    const answer = data?.candidates?.[0]?.content?.parts?.map((p) => p.text).join('').trim();
    res.status(200).json({ answer: answer || '죄송해요, 답변을 만들지 못했어요. 다시 물어봐 주세요.', ok: true });
  } catch (e) {
    console.error('ask 함수 예외', e);
    res.status(200).json({ answer: '⚠️ 오류가 발생했어요. 잠시 후 다시 시도해 주세요.', ok: false });
  }
}
